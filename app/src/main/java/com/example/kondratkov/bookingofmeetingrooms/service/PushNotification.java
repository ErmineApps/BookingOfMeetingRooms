    ...
    private void pushReservation(Reservation reservations){
        try {
            if(reservations.isStatus()) {
                sendNotification("Подтверждение бронировании комнаты ","Подтверждение бронировании комнаты  в "
                        + mDataTimePepresentation.getData_HHmm(reservations.getDateStart(),mContext), "");
                ...
                if(reservations.isStatus()) {
                    sendNotification("Вам отказали в бронировании комнаты ","Вам отказали в бронировании комнаты "+reservations.getMeetingRoom().getName(), "");
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
